...
export async function GET(req: Request) {
  ...
  if (id) {
    // Get by id
    const data = await prisma.expense.findUnique({ where: { id: Number(id) } });
...
export async function POST(req: Request) {
  try {
    ...
    await sendNotificationToAdmins(
      ...
      `Pengeluaran <b>${created.description}</b> sebesar <b>Rp${Number(created.amount).toLocaleString('id-ID')}</b> berhasil ditambahkan ke sistem`,
      "success",
      req
    );
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await sendNotificationToAdmins(
      ...
      `Pengeluaran <b>${updated.description}</b> sebesar <b>Rp${Number(updated.amount).toLocaleString('id-ID')}</b> berhasil diupdate`,
      "info",
      req
    );
...
export async function DELETE(req: Request) {
  ...
  try {
    if (id) {
      const deleted = await prisma.expense.delete({ where: { id: Number(id) } });
      await sendNotificationToAdmins(
        "Pengeluaran Dihapus",
        `Pengeluaran <b>${deleted.description}</b> sebesar <b>Rp${Number(deleted.amount).toLocaleString('id-ID')}</b> berhasil dihapus`,
        "warning",
        req
      );
    ...
    if (Array.isArray(body.ids)) {
      await prisma.expense.deleteMany({ where: { id: { in: body.ids.map(Number) } } });
      await sendNotificationToAdmins(
        "Pengeluaran Dihapus (Bulk)",
        `Beberapa data pengeluaran berhasil dihapus (total: ${body.ids.length})`,
        "warning",
        req
      );
      return NextResponse.json({ success: true });
