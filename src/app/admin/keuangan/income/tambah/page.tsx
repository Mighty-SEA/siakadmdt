...
export default function TambahIncomePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        showActionToast("Pemasukan berhasil ditambahkan", "success");
        router.push(`/admin/keuangan/income?status=success&message=${encodeURIComponent('Pemasukan berhasil ditambahkan')}`);
      } else {
        const data = await res.json();
        showActionToast(data.error || "Gagal menambah pemasukan", "error");
      }
    } catch {
  ...
  return (
    <div className="card bg-base-200 shadow-lg p-6 md:p-10 rounded-2xl border border-primary/30 text-base-content w-full max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-bold text-primary">Tambah Pemasukan</h2>
      </div>
