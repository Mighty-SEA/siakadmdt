...
export default function EditExpensePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (res.ok) {
        ...
        router.push("/admin/keuangan/expense");
      }
      setFetching(false);
    }
    if (id) fetchData();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        showActionToast("Pengeluaran berhasil diupdate", "success");
        router.push(`/admin/keuangan/expense?status=success&message=${encodeURIComponent('Pengeluaran berhasil diupdate')}`);
      } else {
        const data = await res.json();
        showActionToast(data.error || "Gagal mengupdate pengeluaran", "error");
      }
    } catch {
  ...
  return (
    <div className="card bg-base-200 shadow-lg p-6 md:p-10 rounded-2xl border border-primary/30 text-base-content w-full max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-bold text-primary">Edit Pengeluaran</h2>
      </div>
